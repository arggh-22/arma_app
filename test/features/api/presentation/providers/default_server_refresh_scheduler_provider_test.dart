...
void main() {
  group('defaultServerRefreshSchedulerProvider', () {
    test(
      'applyInterval registers periodic task and cancels when disabled',
      () async {
    ...
    test('uses retry ladder 1m, 5m, 15m then stops', () async {
      ...
      final notifier = container.read(
        defaultServerRefreshSchedulerProvider.notifier,
      );

      await notifier.runBackgroundTask(
        task: defaultServerPeriodicRefreshTaskName,
      );
      await notifier.runBackgroundTask(
    ...
    test(
      'checkAndRunOverdueRefresh marks recovered timestamp on success',
      () async {
        SharedPreferences.setMockInitialValues(<String, Object>{});
        final prefs = await SharedPreferences.getInstance();
